...
const writeGitIgnore = (src, dest) => new Promise((resolve, reject) => {
	const srcFile = path.resolve(src, './lib/gitContent.txt');
	const destFile = path.resolve(dest, '.gitignore');
	const writeStream = fs.createWriteStream(destFile);
	fs.createReadStream(srcFile).pipe(writeStream);
	writeStream.on('error', reject);
	writeStream.on('close', () => {
		console.log(chalk.green('create .gitignore done'))
		resolve();
	});
})
	...
	mkdirp(destPath, true, (isExist) => {
		if (isExist) {
			...
			fs.copy(srcTemplatePath, destPath, { filter: filterFunc })
				...
				.then(() => writeGitIgnore(srcPath, destPath))
